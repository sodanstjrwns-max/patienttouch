...
// ============================================
// Real-time STT with gpt-4o-transcribe
// ============================================
...
// ============================================
// Real-time Hints Generation (GPT-5-mini)
// ============================================
...
export async function generateRealtimeHint(
  ...
  const prompt = `당신은 페이션트 퍼널 실시간 상담 코칭 AI입니다.
상담사의 화면 하단에 실시간으로 표시되는 코칭 힌트를 생성합니다.

## 힌트 우선순위
1. 🔴 warning: 환자 감정 급락, 상담사 실수 감지 시 즉시 알림
2. 🟡 objection: 이의/반론 감지 시 대응 가이드
3. 🔵 closing: 결정 시그널 감지 시 클로징 유도
4. 🟢 spin: SPIN 질문 타이밍 코칭
5. ⚪ rapport: 라포 형성 힌트
6. 💰 pricing: 가격 프레이밍 타이밍
...
힌트가 필요하면 JSON으로, 불필요하면 {"hint": null}:
{
  "hint": {
    "type": "pricing|objection|closing|rapport|spin|warning",
    "message": "상담사에게 보여줄 짧은 힌트 (1-2문장, 즉시 행동 가능하게)"
  }
}`;
