# 페이션트 터치 (Patient Touch v9.3.2)

## ✅ v9.3.2 설명서 역검증 & 정합화 (2026-07-22)

설명서의 모든 주장을 코드/프로덕션과 대조하는 역검증 수행. **약 60개 주장 중 11개 불일치 발견 → 전부 수정**.

### 설명서 수정 (GuidePage.tsx, 11건)
1. 온보딩 6단계 → 실제 순서로 교체 (개인정보 보관정책 → 직원 초대 → 첫 환자 → 첫 녹음 → 첫 AI 분석 → 첫 리콜 액션) — 가짜 “목표 설정/브리핑 알림” 삭제
2. 코칭 4항목: 라포 → **이의처리** (ai.ts 실제 프레임워크)
3. 캘린더 월 이동 “스와이프” 삭제 (화살표 버튼만 구현됨)
4. 터치 리포트 구성: “자주 하는 질문” → 실제 스키마(요약·구강상태·치료옵션·상담 중 Q&A·다음단계)
5. 제안서 “유효기간” 삭제 (미구현 필드)
6. 리텐션 분류명: 미동의/부분동의 → 실제 분류(상담 미전환·이탈 위험·치료중·완료·정상·6/12개월 리콜·미예약 주의/긴급) + TOC 문구 수정
7. 성장 추적 영역: 4항목 → 실제 6영역(라포/질문기법/이의처리/가격제시/클로징/상담구조)
8. 원장 대시보드 시간대별 분포 → “추정치 기반, 정밀 집계 예정”으로 정직하게 명시 (프론트 mock 확인됨)
9. 카카오 알림톡: “설정에서 등록” → “서버 측 설정 방식, 문의처 안내” (프론트 설정 UI 없음)
10. 설정 표에서 “목표 설정” 행 삭제 (목표 편집은 리포트 탭에 있음)
11. FAQ: 상담 목록 “내 상담만 필터” → 캘린더 “내 일정만” (my_only는 캘린더에만 UI 존재)

### 코드 수정 (SettingsPage.tsx, 2건)
- 앱 정보 버전 v6.0.0(stale) → v9.3.2
- 앱 정보에 이용약관(/terms)·개인정보처리방침(/privacy-policy) 링크 추가 — 설명서 주장을 실제 구현으로 맞춤

### 검증
- 프로덕션 /guide 200 (0.47s), 수정 문구 8종 렌더 확인, 잔존 오류 키워드 0건, sw `pt-v9.3.2`, /settings에 v9.3.2·약관 링크 반영

## 📖 v9.3.1 사용 설명서 페이지 — /guide (2026-07-19)

### 내용
- **공개 페이지** (로그인 불필요): https://patienttouch.kr/guide
- 가입(30초)부터 전 기능까지 **22개 항목** 순서대로: 가입 → 온보딩 6단계 → 팀 관리 → 환자 → 녹음(동의/분할업로드/파일업로드/분석진행) → AI 분석 읽는 법(8개 카드) → 오늘의 액션 → 캘린더 → 터치 리포트 6단계 라이프사이클 → 치료 제안서 → 리텐션 → AI 이탈 예측/재학습 → 성장 추적·소개 네트워크 → 성과 리포트/목표 → 원장 대시보드(6개 카드) → 카카오 알림톡 → 설정 전체 항목표 → 개인정보·보안 7항목 → 원장 vs 실장 권한표 → 구독/체험 정책 → PWA 설치(iOS/Android) → FAQ 8문항
- 진입점 3곳: 랜딩(/welcome) 네비+푸터, 로그인 화면 하단, 설정 → 앱 정보
- 디자인: 랜딩과 동일한 aurora-dark 톤, 목차 앵커 점프, FAQ 아코디언(details), 권한/설정 테이블

### ✅ 검증
- 프로덕션: /guide 200 (0.47s), 본문 핵심 섹션 렌더 확인, /welcome·/login에 링크 노출, sw `pt-v9.3.1`, 기존 페이지(/welcome /login /register /today) 전부 200 무손상

## 🚀 런칭 전 최종 실사용 시뮬레이션 — 프로덕션 E2E (2026-07-19)

### 시나리오: 신규 병원 원장의 첫날부터 환자 리포트 전달까지 (patienttouch.kr 실서버)
1. **가입**: 신규 병원 등록 → trial 30일 자동 부여, 온보딩 0/6 표시 ✅
2. **팀 구성**: 실장 2명 추가, staff 로그인, staff에게 매출/연락처 마스킹 확인 ✅
3. **상담**: staff가 환자 등록 → 상담 생성(임플란트 450만) → **실제 AI 분석 호출** → 코칭 71점, 결정신호 6.8, 심리 분석/인용/요약 생성, 클로징 태스크 자동 생성(맞춤 메시지 포함) ✅
4. **운영 흐름**: 캘린더(상담일+팔로업), 대시보드 KPI, 대기 태스크 목록 정상 ✅
5. **터치 리포트 풀 사이클**: 카톡수신 동의 → 생성(~20초) → 검증 플래그 4건 해소 → 승인 → 발송(인증번호) → 환자 열람(인증 실패 401→성공, open_count 기록) ✅
6. **보안 재점검**: 미인증 401 / staff→admin API 403 / 교차 테넌트 404×3 ✅
7. **성능**: 페이지 0.13~0.18s, API 평균 0.13~0.30s (글로벌 엣지) ✅

### 발견/확정 사항
- 터치 리포트 생성은 `kakao_delivery` 동의 선행 필수 (CONSENT_REQUIRED) — 개인정보 설계 의도대로 동작
- 환자 공개 열람은 승인만으론 불가, **발송(sent) 후에만 가능** — 워크플로 무결성 확인
- CSRF: JSON content-type 우회는 의도된 설계 (SameSite=Lax 쿠키 + HTML form으로 JSON 전송 불가로 방어)

### 테스트 흔적 완전 삭제 (프로덕션 D1)
- 시뮬레이션 조직 `org_b746a51b`(런칭시뮬레이션치과) + 이전 잔여 테스트 조직 `org_a0799e14`(최종검증치과) — users/patients/consultations/reports/tasks/consents/touch_reports(+revisions/events)/stt_chunks/audit_logs/notification_logs 등 전 테이블 삭제
- 검증: 전 테이블 COUNT 0, 삭제된 리포트 토큰 404, 삭제 계정 로그인 401, 기존 세션 무효
- 잔존 조직 4개(서울BD치과·데모치과×2·문석준의 병원)만 유지 — 실데이터 무손상 확인

## 💳 v9.3.0 구독 게이트 + AI 사용량 패널 (2026-07-18)

### 배경
멀티테넌트 감사(v9.2.1)에서 확인된 마지막 공백 2개 — "trial 30일이 DB에 기록만 되고 아무것도 안 막음", "병원별 AI 원가를 알 수 없어 과금 설계 근거가 없음"을 해소.

### 1) 구독 만료 게이트 — "읽기 전용 모드" (auth.ts)
- **정책**: 만료돼도 데이터는 절대 잠그지 않는다. **조회(GET)는 영구 허용**, 쓰기(POST/PUT/PATCH/DELETE)만 402 차단 → 이탈 병원도 데이터 인질로 잡지 않는 신뢰 설계
- 판정: `status='expired'` 즉시 차단 / `status='trial'` + end_date 경과 시 차단 / `status='active'`는 end_date 무관 허용(수동 결제 관리 유예)
- `/api/auth/*`는 만료 후에도 허용 (로그아웃·설정·팀 확인) / 쓰기 요청에만 PK 단건 조회 (~1ms, GET 성능 영향 0)
- 응답: HTTP 402 + `code: SUBSCRIPTION_EXPIRED` → 프론트 safeFetch가 감지해 토스트 안내
- SQLite datetime 공백 구분 형식("YYYY-MM-DD HH:MM:SS") ISO 정규화 + 파싱 실패 시 차단 안 함(가용성 우선)

### 2) 병원별 AI 사용량/원가 API — `GET /api/dashboard/usage` (admin 전용)
- 월별(YYYY-MM) 집계: AI 분석 상담 수 / 녹음 처리 분 / 터치 리포트 / 이탈 예측 / 카톡 발송
- 원가 추정: STT 분당 $0.006 + 분석 건당 $0.25 + 리포트 $0.08 + 예측 $0.01 (환율 1450원) — 작업 단위 추정치
- 설정 화면에 "이번 달 AI 사용량" 패널 (admin에게만 노출, staff 403)

### 3) 체험판 UX
- `/me`에 `trial_days_left` 추가 → 홈 상단 D-day 배너 (7일 이하 노랑, 만료 빨강 "데이터는 안전하게 보관 중"), 설정 구독 상태에 (D-n) 표시

### ✅ 검증 (E2E)
- 로컬: trial 유효 병원 쓰기 200 → end_date를 어제로 변경 → 쓰기 402 + GET 200 + 로그아웃 200 + D-day 0 반영 ✅ / usage API 정상 집계(데모: 분석 2건·17분 → 약 1,018원) ✅ / staff의 usage 403 ✅ / 테스트 데이터 정리 ✅
- 프로덕션: `/me`에 trial_days_left 필드 ✅ / usage API 200 ✅ / demo 쓰기 정상(만료 아님) ✅ / sw `pt-v9.3.0` ✅
- 프로덕션 데이터 정비: org_demo가 trial 만료 상태(2026-03-01)여서 게이트에 걸릴 뻔 → active/2027-12-31로 갱신 (org_a0799e14는 8/2까지 trial — 정상)
...
### 진단
- 빠른 녹음 모드는 환자 미선택 시 `녹음_MMDD_HHMM` 임시 환자를 자동 생성 — 이후 아무도 실명으로 바꾸지 않으면 캘린더/목록에 파일명처럼 노출
- 확인 결과 프로덕션에 임시명 환자 9명 잔존. 그중 문제의 2건(0718_0153 교정, 0717_1229 어금니)은 **상담 원문에 환자 실명이 한 번도 등장하지 않아** 자동 복원 불가 (교정 건은 "부모님"만 5회)

### 수정 (이중 방어)
1. **NER에 patient_name 추출 추가** (`ai-presenter.ts`): 상담 중 "──님"으로 명시적으로 불린 실명만 추출, 호칭(부모님/원장님 등)은 제외, 불확실하면 null
2. **분석 파이프라인 자동 개명** (`analysis-runner.ts` extracting 단계): 환자명이 `녹음_MMDD_HHMM` 패턴 && NER이 유효한 한글 2-4자 실명 추출 && 호칭 블랙리스트 통과 시에만 `patients.name` 자동 교체 (name 일치 조건부 UPDATE로 레이스 방지, 실패해도 분석은 계속)
3. **캘린더 표시 정리** (`calendar.js`): 임시명은 "미지정 환자" + 🔗연결 필요 배지로 표시 — 파일명 노출 차단, 클릭하면 상담 상세의 [환자 연결] 흐름으로 자연 유도

### ✅ 검증 (실사용 시뮬레이션)
- 로컬 양성: 임시 환자 생성 → "김민준님" 3회 포함 스크립트 주입 → extracting 폴링 1회 → `patients.name`이 **녹음_0718_0448 → 김민준** 자동 교체 확인 (NER patient_name=김민준)
- 로컬 음성: 이름 없는 스크립트("부모님"만 언급) → 임시명 유지 확인 (오탐 없음)
- 프로덕션: 배포된 calendar.js를 내려받아 원장님 화면 데이터 그대로 렌더 시뮬 — "미지정 환자" 2건 + 연결 필요 배지 2건, 녹음_ 파일명 노출 0건
- 기존 잔존 2건은 원문에 실명이 없어 자동 복원 불가 → 캘린더/상담 상세에서 [환자 연결]로 수동 연결 필요
- sw `pt-v9.1.7`
...
### 진단
- 만들기 클릭 → 상담 상세 카드가 **클릭 불가능한 "생성중" 스피너 배지**로 바뀜 → 사용자는 누를 곳이 없음 ("눌러도 볼 수가 없어")
- 게다가 상태 폴링 fetch가 한 번이라도 실패하면 catch에서 `el.innerHTML=''` — 액션 영역이 **영구 공백**
- 실제 리포트 데이터는 D1에 정상 생성돼 있었음 (원장님 org 리포트 e9bebad2… status=review, content 존재)

### 수정
1. **만들기 → 즉시 검수 화면으로 이동**: 생성 시작되면 `/touch-reports/:id/review`로 리다이렉트 — 검수 화면이 자체 폴링(3초)으로 진행상황 표시 후 완료 시 자동 렌더
2. **"생성중" 배지를 클릭 가능한 링크로**: 생성중에도 "생성중 · 보기"로 검수 화면 진입 가능 (폴링이 poll-to-advance로 생성을 전진시킴)
3. **폴링 실패 시 공백 대신 [상태 확인] 재시도 버튼** + 5초 후 자동 재시도

### ✅ 검증 (실사용 시뮬레이션)
- 로컬: 동의 기록 → generate → 검수 페이지 200 → 폴링 1회 만에 review+content 완료
- 프로덕션: 실제 generate → 폴링 → review 완료, 검수 페이지 HTML/JS 정상 서빙, 원장님 org 리포트 API 조회 정상(review+content). 검수 JS를 실데이터(flags 3건 포함)로 DOM 렌더 시뮬레이션 — 요약/치료옵션/확인필요 배지 전부 렌더 확인
- sw `pt-v9.1.5`
