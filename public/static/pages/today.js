    ...
    topEl.innerHTML =
      '<div class="flex items-center gap-2.5">' +
        ...
        '<div class="flex-1 min-w-0">' +
          ...
          '<p class="text-sm font-bold text-white truncate">' + (top.patient_id ? PT.patientNameLink(top.patient_id, top.patient_name, {cls:'underline decoration-dotted decoration-white/40 underline-offset-2 active:opacity-60'}) : esc(top.patient_name)) + '님' +
...
function renderTpChecklist() {
  ...
  items.forEach(function(it, idx) {
    ...
    if (it.patient_id && it.patient_name) html += PT.transcriptBtn(it.patient_id, it.patient_name);
