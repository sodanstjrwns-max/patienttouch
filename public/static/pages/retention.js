...
function renderDashboard(d) {
  ...
  if (d.today_contacts.length === 0) {
    ...
    d.today_contacts.forEach(function(c) {
      ...
      html += '<div class="w-10 h-10 rounded-xl ' + avatarColor + ' flex items-center justify-center font-bold text-sm shrink-0">' + esc(c.patient_name).charAt(0) + '</div>';
      html += '<div class="flex-1 min-w-0">';
      html += '<div class="flex items-center gap-2"><a href="/patients/' + c.patient_id + '" class="font-bold text-sm text-surface-900 hover:text-brand-600 transition-colors">' + esc(c.patient_name) + '</a>';
      html += '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold ring-1 ring-inset ' + st.bg + ' ' + esc(st.text) + ' ' + st.ring + '"><i class="fas ' + st.icon + '"></i>' + st.label + '</span></div>';
  ...
  if (riskGroups.critical.length > 0) {
    ...
    riskGroups.critical.slice(0, 3).forEach(function(p) {
      html += '<div class="flex items-center justify-between">' +
        '<a href="/patients/' + p.patient_id + '" class="text-xs font-semibold text-rose-800 hover:underline">' + esc(p.patient_name) + '</a>' +
  ...
  if (r.staff_stats && r.staff_stats.length > 0) {
    ...
    r.staff_stats.forEach(function(s, i) {
      ...
      html += '<div class="flex items-center justify-between p-3 bg-surface-50 rounded-xl">' +
        '<div class="flex items-center gap-2"><span class="text-base">' + medal + '</span><span class="font-bold text-sm text-surface-900">' + esc(s.staff_name) + '</span></div>' +
